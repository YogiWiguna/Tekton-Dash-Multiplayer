...
func _ready():
	# Initially disable the host and join buttons.
	host_button.disabled = true
	join_button.disabled = true

	# Connect signals from UI elements to their respective functions.
...
func _on_username_text_changed(new_text):
	# Enable the host and join buttons only if the username is not empty.
...
func _on_host_button_pressed():
	...
	# Reload the main scene for the host.
...
func _on_connection_result(success: bool):
	...
	if success:
		...
		# The client successfully connected, now change the scene.
		get_tree().change_scene_to_file("res://scenes/main.tscn")
	else:
		print("Connection failed. Please check the IP address and try again.")
		# Here you could show an error message to the user in the UI.
